...
import { getAllDrinks, getAllDrinkCategories, createDrink, updateDrink, deleteDrink, createDrinkCategory } from "@/lib/actions/pos";
...
export default function DrinksPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = editingItem
      ? await updateDrink({ ...formData, id: editingItem.id, image: imageUrl })
      : await createDrink({ ...formData, image: imageUrl });

    if (result.success) {
      toast.success(editingItem ? "Drink updated" : "Drink created");
      setIsDialogOpen(false);
      resetForm();
      loadData();
    } else {
      toast.error(result.error || "Failed to save drink");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.success) {
      ...
      toast.error(result.error || "Failed to delete drink");
    }
  };
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = await createDrinkCategory(newCategoryData);
    if (result.success) {
      toast.success("Category created");
      setIsAddCategoryDialogOpen(false);
      setNewCategoryData({ name: "", description: "", displayOrder: 0 });
      // Reload categories
      const categoriesResult = await getAllDrinkCategories();
      if (categoriesResult.success) {
        setCategories(categoriesResult.categories || []);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage drinks</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingItem ? "Edit" : "Add"} Drink</DialogTitle>
            </DialogHeader>
                  ...
                  <Dialog open={isAddCategoryDialogOpen} onOpenChange={setIsAddCategoryDialogOpen}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>Create a new drink category</DialogDescription>
                      </DialogHeader>
