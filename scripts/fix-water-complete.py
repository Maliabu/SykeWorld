"""
Fix logo (1).png - ensure ALL water areas are navy blue
Check for any blue-ish colors that should be navy blue
"""
...
def find_circle_center_and_radius(img):
    ...
    # Use the larger radius to ensure we cover the full circle
...
def is_blueish_color(r, g, b):
    """Check if color is blue-ish (could be water)"""
    # Blue should be the dominant color
    if b > r and b > g:
        return True
    # Or if it's a dark blue-ish color
...
def fix_all_water_areas(img):
    """Make sure all water areas inside circle are navy blue"""
    pixels = img.load()
    width, height = img.size
        ...
        for x in range(width):
            ...
            if is_inside_circle(x, y, center_x, center_y, radius):
                ...
                if (r, g, b) == vibrant_green:
                    already_green += 1
                # Keep navy blue (already correct)
                elif (r, g, b) == navy_blue:
                    already_navy += 1
                # Convert any blue-ish colors to navy blue
                elif is_blueish_color(r, g, b):
    ...
    print(f"\nResults:")
    print(f"  Green (continents): {already_green}")
    print(f"  Already navy (water): {already_navy}")
    print(f"  Blue-ish converted to navy: {blueish_to_navy}")
    print(f"  Other colors converted to navy: {converted_to_navy}")
    print(f"  Total water pixels: {already_navy + blueish_to_navy + converted_to_navy}")
...
if __name__ == "__main__":
    ...
    print("\nFixing ALL water areas to be navy blue...")
    img = fix_all_water_areas(img)
    
    print(f"\nSaving updated logo (1).png...")
