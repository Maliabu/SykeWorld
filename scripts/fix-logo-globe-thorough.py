"""
Fix logo (1).png more thoroughly:
1. Find the perfect circle (globe)
2. Convert ALL orange parts inside circle to green (continents)
...
def is_orange_pixel(r, g, b):
    """More thorough orange detection - catch all shades"""
    # Method 1: High red, medium green, low blue
    if r > 120 and g > 30 and b < 120:
        if r > g and r > b:
            return True
    
    # Method 2: Orange-like colors (reddish-yellow)
    if r > 180 and g > 80 and b < 100:
        return True
    
    # Method 3: Check hue - orange is between red and yellow
    # Simplified: high red, medium-high green, low blue
    if r > 150 and 50 <= g <= 200 and b < 150:
        # Red should be dominant
        if (r - g) > 20 and (r - b) > 50:
            return True
    
    # Method 4: Very specific orange shades
...
def fix_globe_colors(img):
    """Fix colors inside the globe circle - more thorough"""
    pixels = img.load()
    width, height = img.size
    ...
    for y in range(height):
        for x in range(width):
            ...
            if is_inside_circle(x, y, center_x, center_y, radius):
                # Check if it's already green - keep it
                if r == 34 and g == 139 and b == 34:
                    already_green += 1
                # Check if it's orange (continent) - convert to green
                elif is_orange_pixel(r, g, b):
                    pixels[x, y] = (*vibrant_green, a)
                    orange_to_green += 1
                # Check if it's already navy blue - keep it
    ...
    print(f"Already green (kept): {already_green}")
...
if __name__ == "__main__":
    ...
    print("Fixing globe colors (thorough pass)...")
